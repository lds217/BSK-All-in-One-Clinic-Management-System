<#
.SYNOPSIS
    Checks for a new version of the server JAR application, downloads it, and replaces the old version.

.DESCRIPTION
    This script is dedicated to updating the server application.
    It reads a local version_server.json file to determine the current version.
    It then fetches a remote latest_server.json file from a specified URL to see if a newer version is available.
    If an update is found, it stops the running server, downloads the new JAR,
    backs it up, installs the new one, and restarts the server.

.NOTES
    Author: Dat Le
    Date: 2025-09-07
    Version: 1.1.0 (Server Only)
#>

# --- CONFIGURATION ---
# IMPORTANT: This URL should now point to your 'latest_server.json' file on GitHub.
...
# Define the paths for the local files (now server-specific)
$localVersionFile = Join-Path $scriptPath "version_server.json"
...
Write-Host "Reading local server version info from '$localVersionFile'..."
...
try {
    ...
    Write-Host " - Local Server Version: $($localVersionInfo.currentVersion)"
}
catch {
    Write-Error "Error: Could not read or parse the local version_server.json file. Is it valid JSON?"
    Read-Host "Press Enter to exit"
    exit 1
}
...
Write-Host "Fetching remote server version info from '$remoteVersionInfoUrl'..."
try {
...
if ([version]$localVersionInfo.currentVersion -ge [version]$remoteVersionInfo.latestVersion) {
    Write-Host "`nResult: Your server application is up-to-date! (Version $($localVersionInfo.currentVersion))"
}
else {
    ...
    $downloadUrl = $remoteVersionInfo.files.server
    if (-not $downloadUrl) {
    ...
    try {
        # This command finds the java process running 'server.jar' and stops it.
    ...
    $tempDownloadPath = Join-Path $scriptPath "server.jar.download"
    ...
    try {
        ...
        if (Get-Command Start-BitsTransfer -ErrorAction SilentlyContinue) {
            Start-BitsTransfer -Source $downloadUrl -Destination $tempDownloadPath -DisplayName "Downloading server.jar" -Description "BSK Server Update"
        }
        else {
            ...
            Register-ObjectEvent -InputObject $webClient -EventName DownloadProgressChanged -Action {
                ...
                Write-Progress -Activity "Downloading server.jar" -Status "$percent% Complete" -PercentComplete $percent -CurrentOperation "Downloaded: $speed MB / $([math]::Round($total/1MB, 2)) MB"
            } | Out-Null
            
            try {
                $webClient.DownloadFile($downloadUrl, $tempDownloadPath)
                Write-Progress -Activity "Downloading server.jar" -Completed
            }
            finally {
                $webClient.Dispose()
    ...
    # Example: Start-Process ".\start_server.bat"
